...
var helpers = require('../lib/helpers.js'),
	...
	Prefix = require('../lib/entities/measurement_system.js');

// MEASUREMENT OBJECT
// Shorthand creation of Quantity

var measurement = function (value, systemName, unitName) {
	return new measurement.Quantity(value, systemName, unitName);
};

// EXPOSED CLASSES

measurement.Quantity = null;
measurement.Dimension = null;
...
measurement.resetToDefaultOptions = function() {
	var optionPath = '../lib/options.js';
	delete require.cache[optionPath];
	measurement.options = new require(optionPath);
}
...
measurement.load = function(dataObject) {
	...
	// These need to be populated after loading to make sure
	// that the circular dependency allows measurement to be populated.
	measurement.Quantity = require('../lib/quantity.js');
	measurement.Dimension = require('../lib/dimension.js');
...
function findImpl(items, matcher, findParam, ignoreCase, filterFunc, sortCalculatePoints) {
	// Validate
	if (!helpers.isString(findParam) || findParam.length == 0) {
...
function allowedSystems() {
	...
	var toRemove = [];
	helpers.forEach(measurement.ignoredSystemsForUnits, function (ignore) {
...
function possibleAllowedSystems() {
	var results = measurement.options.allowedSystemsForUnits.filter(function (system) {
...
measurement.findUnits = function (unitName, dimensionName, ignoreCase) {
	var units = (dimensionName) ? measurement.findDimension(dimensionName, ignoreCase) : measurement.allUnits;
...
measurement.findPrefixesPartial = function () {
